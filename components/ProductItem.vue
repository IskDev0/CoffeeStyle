<template>
  <article @click="$router.push(`products/${product.id}`)" class="flex flex-col items-center text-center cursor-pointer relative">
    <span v-if="product.discount" class="absolute top-0 right-0 bg-red-400 text-white font-bold py-2 px-4">-{{product.discount}}%</span>
    <img class="h-96 w-80" :src="product.image" :alt="product.title">
